...
class LSTMShakespeareDataset(Dataset):
    '''
    ...
    def __init__(self,
        ...
        '''
        Initializes a LSTMShakespeareDataset object.
    ...
    def __getitem__(self, index: slice) -> List[Tuple[Union[torch.Tensor, Iterable], int]]:
        ...
        if isinstance(index, slice):
            if self.to_tensors:
                return [(torch.tensor(sample[0], dtype=torch.int64), sample[1]) for sample in self.samples[index]]
            return [(sample[0], sample[1]) for sample in self.samples[index]]
        else:
            if self.to_tensors:
                return (torch.tensor(self.samples[index][0], dtype=torch.int64), self.samples[index][1])
    ...
    def __str__(self) -> str:
        ...
        '''
        return f'LSTMShakespeareDataset(vocab_size: {self.vocab_size}, block_size: {self.block_size}, to_tensors: {self.to_tensors}, device: {self.device})'
    ...
    @staticmethod
    def _normalize(content: str, contractions_dict: Dict[str, str], punct_list: str) -> str:
        '''
        ...
        Args:
            ...
            punct_list (str): String containing punctuation characters to remove.
    ...
    def _proc_file(self, file: str, contractions_dict: Dict[str, str], punct_list: str) -> str:
        '''
        ...
        content = LSTMShakespeareDataset._normalize(content, contractions_dict, punct_list)
    ...
    def _normalize_data(self) -> None:
        ...
        for file_name in os.listdir(self.dataset_path):
            LSTMShakespeareDataset.logger.info(f'Processing dataset: {os.path.join(self.dataset_path, file_name)}.')
            # Normalize the content of the file
            norm_text = self._proc_file(file_name, contractions_dict, string.punctuation)

            if norm_text == '':
                LSTMShakespeareDataset.logger.info(f'File {file_name} is not a .txt file, so it\'s been ignored.')
                continue

            self._write_norm_file(norm_text, file_name=file_name)
            LSTMShakespeareDataset.logger.info(f'Normalized dataset succesfully saved to: {os.path.join(self.norm_dataset_path, file_name)}')
    ...
    def _create_samples(self):
        ...
        Returns:
            List[Tuple[List[int], int]]: List of training samples, where each sample is a tuple of
        ...
        for i in tqdm(range(len(data) - self.block_size), ascii=True, desc='Creating Samples'):
            samples.append((data[i:i+self.block_size], data[i+self.block_size]))

        LSTMShakespeareDataset.logger.info('Samples created succesfully.')
    ...
    def encode(self, input_string: str) -> List[int]:
        ...
        norm_input = self._normalize(input_string, contractions_dict, punct_list)
