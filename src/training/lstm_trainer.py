...
class LSTMTrainer:
    ...
    def __init__(self,
        ...
        '''
        Initializes the LSTMTrainer with the model, data loaders, loss function, evaluation function, and optimizer.
    ...
    def fit(self,
        ...
        LSTMTrainer.logger.info('Start Training Process.')

        # Get the loaders if the user doesn't want to use cross validation
        if not cross_validate:
            LSTMTrainer.logger.info('Creating training and validation DataLoaders.')
            train_dl, valid_dl = self._get_loaders()
            LSTMTrainer.logger.info('Dataloaders created succesfully.')

        for epoch in range(1, epochs + 1):
            LSTMTrainer.logger.info(f'-> Epoch: {epoch}/{epochs}')

            # Get the loaders if the user want to use cross validation
            if cross_validate:
                LSTMTrainer.logger.info('    Creating training and validation DataLoaders (cross-validation step)')
            ...
            # Log the results
            LSTMTrainer.logger.info('')
            LSTMTrainer.logger.info(f'    Results (lr={self.opt.param_groups[0]["lr"]:.6f}):')
            LSTMTrainer.logger.info(f'    Train Loss:       {train_loss:.4f}')
            LSTMTrainer.logger.info(f'    Train Eval Score: {train_eval:.4f}')
            LSTMTrainer.logger.info(f'    Valid Loss:       {valid_loss:.4f}')
            LSTMTrainer.logger.info(f'    Valid Eval Score: {valid_eval:.4f}')
            ...
            if self.scheduler is not None:
                self.scheduler.step()
                LSTMTrainer.logger.info('')
                LSTMTrainer.logger.info(f'    Scheduling step executed succesfully (new lr={self.opt.param_groups[0]["lr"]:.6f})')
            ...
            LSTMTrainer.logger.info(('-' * 100))

        LSTMTrainer.logger.info('Training Process Completed Successfully.')
